...
export default class Board {
    ...
    public static handleEvent(direction: string){
        this.OldIndex.length = 0;
        Board.Matrix.forEach(blocks => blocks.forEach(block => {
            if(block) this.OldIndex.push(block.currentValue);
            else this.OldIndex.push(0);
        }));
